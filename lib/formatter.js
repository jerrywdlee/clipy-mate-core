...
class Fmt {
  ...
  */
  ...
  static async parseXml(xmlString) {
    if (!xmlString) {
      throw Error('No XML found!');
    }
    const rawObj = await parseString(xmlString);
    return xmlObj2FolderObj(rawObj);
  }
...
function xmlObj2FolderObj(xmlObj) {
  const folders = xmlObj['folders']['folder'].map(fldObj => {
    const folder = { title: fldObj['title'][0] };
    ['identifier', 'index'].forEach(key => {
      if (fldObj[key]) {
        folder[key] = fldObj[key][0];
      }
    });
    folder['snippets'] = fldObj['snippets'][0]['snippet'].map(xmlObj2SnippetObj);
    return folder;
  });
  return folders;
}

function xmlObj2SnippetObj(snpObj) {
  const snippet = {
    title: snpObj['title'][0],
    content: snpObj['content'][0],
  };
  ['identifier', 'index'].forEach(key => {
    if (snpObj[key]) {
      snpObj[key] = snpObj[key][0];
    }
  });
  return snippet;
}
